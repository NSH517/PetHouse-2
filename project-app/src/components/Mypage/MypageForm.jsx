...
const PetSection = ({onPetUpdated}) => {
  ...
  const handleEdit = async () => {
    ...
    try { await axios.post('/api/pets/update', fd, { headers: getAuthHeader() }); alert('반려견 정보가 수정되었습니다.'); setEditOpen(false); loadPets(); onPetUpdated() }
...
const MypageForm = () => {
  ...
  return (
    <div style={{ backgroundColor: '#f5f5f5', minHeight: '100vh', fontFamily: "'Noto Sans KR', system-ui, sans-serif" }}>
      ...
      <UserSection />
      <GradeCouponSection />
      <PetSection onPetUpdated={refresh} />
